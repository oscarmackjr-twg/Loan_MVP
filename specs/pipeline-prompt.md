...
pytest backend/tests/test\_pipeline\_routes.py -v --tb=short

ruff check backend/api/ backend/pipeline/
...
Split into these chunks with shared context:
...
| 2a | backend/pipeline/eligibility.py | Eligibility rules + run\_eligibility\_checks() |

| 2b | backend/pipeline/phases.py | All 7 phase implementations |

| 2c | backend/pipeline/engine.py | Pipeline orchestrator |

| 2d | backend/pipeline/notebook\_outputs.py | CSV output generation helpers (if separated from phases.py) |

| 2e | backend/api/routes.py | All API route implementations |

| 2f | backend/api/dependencies.py | Storage dependency |

| 2g | backend/tests/test\_pipeline\_routes.py | Full test suite |



Prepend specs/context/project-context.md + specs/context/phase1-output-summary.md
